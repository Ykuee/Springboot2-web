...
public class SysRoleController {
	...
	@GetMapping("/getRoles")
	public List<SysRoleDTO> getRoles(SysRoleDTO dto)  {
		return sysRoleService.getRoleList(dto);
	}
	...
	public void delRole(@RequestBody SysRoleDTO sysRoleDTO) {
		...
		if(!success) {
			throw new BusinessException("删除权限时发生错误");
		}
	}
	...
	@SysLog(
			description = "删除用户所有权限",
			bizType = LogTypeConstant.SYSTEMSERVICE,
			bizTypeDescription = LogTypeConstant.SYSTEMSERVICE_NAME,
			operateType = LogConstant.LogOperateType.DELETE,
			operateTypeDescription = "删除用户所有权限"
	)
	@PostMapping("/delUserAllRoles")
	public void delUserAllRoles(@RequestBody SysUserRoleDTO dto) {
		boolean success = sysUserRoleService.delUserAllRoles(dto.getUserId());
		if(!success) {
			throw new BusinessException("删除用户权限时发生错误");
		}
	}
