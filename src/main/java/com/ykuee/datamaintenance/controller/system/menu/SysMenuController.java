...
public class SysMenuController {
	...
	@SysLog(
			description = "为用户删除所有已分配菜单",
			bizType = LogTypeConstant.SYSTEMSERVICE,
			bizTypeDescription = LogTypeConstant.SYSTEMSERVICE_NAME,
			operateType = LogConstant.LogOperateType.UPDATE,
			operateTypeDescription = "为用户删除所有已分配菜单"
	)
	@PostMapping("/delRoleAllMenu")
	public void delRoleAllMenu(@RequestBody SysRoleMenuDTO dto) {
		boolean success = sysRoleMenuService.delRoleAllMenu(dto.getRoleId());
		if(!success) {
			throw new BusinessException("删除菜单时发生错误");
		}
	}
